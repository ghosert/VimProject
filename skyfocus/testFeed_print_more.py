            ...
            if self.__currentEntry == None:
                ...
                for newEntry in atom.entries:
                    if convertGmailPublished(newEntry.published) > convertGmailPublished(self.__currentEntry.published):
                        print 'add newEntry: ' + str(newEntry.published_parsed)
                ...
                if len(newlist) <= 0:
                    return # Quit if no new entry.
                
                # Set the newest mail entry to current entry
                self.__currentEntry = newlist[0]
                print 'set current entry published_parsed: ' + str(self.__currentEntry.published_parsed)
                print 'set current entry updated_parsed: ' + str(self.__currentEntry.updated_parsed)
                print 'set current entry published: ' + str(self.__currentEntry.published)
