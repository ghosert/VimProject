...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
     ...
     case WM_PAINT:
          ...
          COLORREF color = RGB(0, 255, 0); // define a color
          SetTextColor(hdc, color);  // set hdc color
          BYTE g = GetGValue(color); // get 255
          ...
          // restore dc to the status before. means output last sentence with black color instead of green color.
          RestoreDC(hdc, -1);
          
          TCHAR *szSentence = "BITSPIXEL: Color bits per pixel: iColors = 1 << (PLANES * BITSPIXEL) = or != NUMCOLORS";
          
          TextOut (hdc, 0, cyChar * i, szSentence, lstrlen(szSentence)) ;
