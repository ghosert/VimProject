...
PARAMS, *PPARAMS ; // Threads shared parameters. Because of this parameter, we don't need global parameter any more.
...
// PVOID will be casted to PPARAMS as a threads shared parameter, it is the same as the third parameter of _beginthread function.
void Thread1 (PVOID pvoid) 
{
     ...
     // it's same to java to stop a thread. if you can't stop a thread normally, you can use KillThread forced.
     // Here is a thread safe problem, while ui thread has set bKill to TRUE, but the code has already passed check. It's not a big deal here.
     // Using EnterCriticalSection - LeaveCriticalSection to solve the problem like synchronized in java.
     // And make sure the threads must not share a gdi object like brush, pen, font, bitmap.
     while (!pparams->bKill)
     {
          if (iNum < 0)
               iNum = 0 ;
     ...
     _endthread () ; // invoke _endthread function here when break loop clause.
}
...
LRESULT APIENTRY WndProc1 (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
     ...
     case WM_CREATE:
          ...
          _beginthread (Thread1, 0, &params) ; // It's threads shared parameter, and same to the parameter of void Thread1 (PVOID pvoid) method.
          return 0 ;
     ...
     case WM_DESTROY:
          params.bKill = TRUE ; // when the ui thread is killed, kill the backgroud thread as well.
          return 0 ;
     }
