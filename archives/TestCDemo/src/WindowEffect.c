...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
     ...
     {
     case WM_CREATE:
    	  CreateAnimateWindows(hwnd, 2000, AW_BLEND | AW_ACTIVATE, TRUE);
    	  // CreateTransParentWindows(hwnd, RGB(255, 0, 0), 192, LWA_ALPHA);
          // because of the function above, the WM_PAINT will not be called. So we add InvalidateRect function here.
     ...
     case WM_PAINT: // move window, other window cover this window, minimize, maximize will force the client area to be valid, then windows sytem call WndProc with WM_PAINT message to redraw the window.
    	  ...
    	  PTSTR ptstr = TEXT("What are you doing here !");
    	  TextOut(hdc, 0, 0, ptstr, lstrlen(ptstr));
    	  ReleaseDC(hwnd, hdc);
    	  
          return 0 ;
          
     case WM_CLOSE:
    	 CreateAnimateWindows(hwnd, 2000, AW_BLEND | AW_ACTIVATE, FALSE);
         break; // break not return 0, or it will prevent windows to send WM_DESTROY message.
