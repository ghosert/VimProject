                    ...
                    PSTR szCmdLine, int iCmdShow)
{
     ...
     // self-define a width and height.
     FindWindowSize (&cxWindow, &cyWindow) ;
     
     hwnd = CreateWindow (szAppName, TEXT ("What Color"),
                          WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_BORDER, // without minimize and maximize button
...
void FindWindowSize (int * pcxWindow, int * pcyWindow)
{
     ...
     // Get the desktop font, text metrics.
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
     ...
     {
     case WM_CREATE:
    	 // Get a desktop dc and ready to operate on it.
     ...
     case WM_TIMER:
          ...
          // get a current position color with the desktop scope.
          cr = GetPixel (hdcScreen, pt.x, pt.y) ;

          // set a current position on screen with black color.
