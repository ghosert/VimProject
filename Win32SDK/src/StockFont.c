...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
     ...
     stockfont [] = { OEM_FIXED_FONT,      "OEM_FIXED_FONT", // TrueType font.
                      ANSI_FIXED_FONT,     "ANSI_FIXED_FONT",    
                      ANSI_VAR_FONT,       "ANSI_VAR_FONT",
                      SYSTEM_FONT,         "SYSTEM_FONT", // Î»Í¼×Ö
                      DEVICE_DEFAULT_FONT, "DEVICE_DEFAULT_FONT", // Ê¸Á¿×Ö
     ...
     case WM_PAINT:
          ...
          SelectObject (hdc, GetStockObject (stockfont[iFont].idStockFont)) ; // Select one of the font.
          GetTextFace (hdc, LF_FACESIZE, szFaceName) ; // Get the face name which belongs to the font has been selected.
          GetTextMetrics (hdc, &tm) ; // Get the metrics which is decided by font selected.
          cxGrid = max (3 * tm.tmAveCharWidth, 2 * tm.tmMaxCharWidth) ; // Get the grid size which is decided by tm above.
