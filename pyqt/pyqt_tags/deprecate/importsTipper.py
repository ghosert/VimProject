...
import pyqt_tags
tags = pyqt_tags.PyQtTags('classInfoMap.qt')
...
def GenerateTip(data):
    ...
    # Jiawei Zhang ADD START
    #return file, (name, doc, args, type) (from the TYPE_* constants)
    #if 'PyQt4' in data:
        #return 'c:\\file.pyd', [('aboutQt', 'aboutQtDescription', '(string)', '2')]

    #tips.insert(0, ('__debug_plugin__', data, '', '2')) # THIS LINE IS WRITTEN FOR DEBUG PURPOSE.

    #import time
    #time.sleep(5) # TEST WHETHER PYDEV WILL INVOKE THIS PY FILE EACH TIME WHEN CODE COMPLETION ACTION HAPPENS.
                    ...
                    if inspect.ismethod(obj) or inspect.isbuiltin(obj) or inspect.isfunction(obj) or inspect.isroutine(obj):
                        try:
                            ...
                            try:
                                ...
                                # obj_to_complete is PyQt class like QApplication
                                # d is a string of method name like 'aboutQt'
                                if len(doc) <= 0 and obj_to_complete.__module__.find('PyQt4') == 0:
                                    # We are passing the arguments like '(QApplication, aboutQt)' below
                                    # we are expected to get:
                                    # args like: '(int, QAction_action)'
                                    # doc string like: 'QWidget QApplication.widgetAt (int x, int y) \n Detail description.'
                                    # Possible Return: (args, doc) or None if no available value, or ('(Please produce the tag file first.)', 'Please produce the tag file first.')
                                    argsDoc = tags.getArgsDoc(obj_to_complete.__name__, d)
                                    if argsDoc:
                                        args, doc = argsDoc
